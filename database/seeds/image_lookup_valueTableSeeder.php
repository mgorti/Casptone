...
class image_lookup_valueTableSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        DB::table('image_lookup_value')->truncate();


        //Load tab-delimited file for medications
        if (($handle = fopen ( public_path('picture_aid_list.txt'), 'r' )) !== FALSE) {
       ...
       /* foreach($select_medlist as $data){
        ...
        for($i=0;$i<count($array1);$i++){
            DB::table('image_lookup_value')->insert(
                [
                    'image_lookup_value_tag' =>  $array1[$i],
                    'image_lookup_value_link' => $array2[$i],
                    'created_by'=>1
                ]);
        }
        //
