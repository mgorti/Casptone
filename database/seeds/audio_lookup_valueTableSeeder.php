...
class audio_lookup_valueTableSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        ...
        DB::table('audio_lookup_value')->truncate();


        //Load tab-delimited file for medications
        if (($handle = fopen ( public_path('audio_list.txt'), 'r' )) !== FALSE) {
...
/*
        ...
        for($i=0;$i<count($array1);$i++){
            DB::table('audio_lookup_value')->insert(
                [
                    'audio_lookup_value_tag' =>  $array1[$i],
                    'audio_lookup_value_link' => $array2[$i],
                    'created_by'=>1
                ]);
        }
        //
